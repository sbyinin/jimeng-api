...
- 🌐 **国际站支持**: 支持即梦国际站（dreamina）的文生图、图生图和视频生成 API，国际站现已支持 Seedance 视频模型，有问题提 issue
- 🔄 **智能轮询**: 自适应轮询机制，优化生成效率
...
> - **马来西亚站**: 需要添加 **my-** 前缀，如 `Bearer my-your_session_id`
>
...
> - **香港/日本/新加坡/马来西亚站 (hk-/jp-/sg-/my-)**: 强制使用 **1k** 清晰度，但支持自定义 ratio 参数（如 16:9、4:3 等）
...
**请求参数**:
- `model` (string, 可选): 使用的模型名称。国内站和国际站（US/HK/JP/SG/MY）均默认 `jimeng-4.5`。
...
- `nanobanana`: 仅国际站支持
- `jimeng-5.0`: 国内站及亚洲国际站 (HK/JP/SG/MY) 支持
- `jimeng-4.6`: 国内站及亚洲国际站 (HK/JP/SG/MY) 支持
...
# 新加坡站使用 "sg-YOUR_SESSION_ID"
# 马来西亚站使用 "my-YOUR_SESSION_ID"
...
4. **全能模式（Omni Reference）**（新）：混合图片+视频作为参考素材，在 prompt 中通过 `@字段名` 引用素材并描述其作用。支持 `jimeng-video-seedance-2.0` 和 `jimeng-video-seedance-2.0-fast` 模型，国内站与国际站均可用。
...
> - 需要 `functionMode=omni_reference`，且 `model` 为 `jimeng-video-seedance-2.0` 或 `jimeng-video-seedance-2.0-fast`。国内站、美国站与亚洲国际站 (HK/JP/SG/MY) 均可使用。
> - **素材数量限制**：
...
**支持的视频模型**:
- `jimeng-video-seedance-2.0` - Seedance 2.0，国内/国际站均支持，支持4~15秒时长，支持全能模式 (Omni Reference) **（最新）**
- `jimeng-video-seedance-2.0-fast` - Seedance 2.0 Fast，国内/国际站均支持，支持4~15秒时长，支持全能模式 (Omni Reference)，速度更快
- `jimeng-video-3.5-pro` - 专业版v3.5，国内/国际站均支持 **（默认）**
- `jimeng-video-veo3` - Veo3模型，仅亚洲国际站 (HK/JP/SG/MY) 支持，固定8秒时长
- `jimeng-video-veo3.1` - Veo3.1模型，仅亚洲国际站 (HK/JP/SG/MY) 支持，固定8秒时长
- `jimeng-video-sora2` - Sora2模型，仅亚洲国际站 (HK/JP/SG/MY) 支持
- `jimeng-video-3.0-pro` - 专业版，国内站和亚洲国际站 (HK/JP/SG/MY) 支持
- `jimeng-video-3.0` - 标准版，国内/国际站均支持
- `jimeng-video-3.0-fast` - 极速版，国内站和亚洲国际站 (HK/JP/SG/MY) 支持
- `jimeng-video-2.0-pro` - 专业版v2，国内站和亚洲国际站 (HK/JP/SG/MY) 支持
- `jimeng-video-2.0` - 标准版v2，国内站和亚洲国际站 (HK/JP/SG/MY) 支持

> **注意**: 美国站现已支持 `jimeng-video-seedance-2.0`、`jimeng-video-seedance-2.0-fast`、`jimeng-video-3.5-pro` 和 `jimeng-video-3.0` 模型。
...
| 马来西亚站 + SOCKS5代理 | `socks5://127.0.0.1:1080@my-session_id_xxx` |
